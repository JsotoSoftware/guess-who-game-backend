...
import-json:
	@powershell -Command "$$dsn = (Get-Content .env | Select-String '^POSTGRES_DSN=').Line -replace '^POSTGRES_DSN=', ''; if ([string]::IsNullOrEmpty($$dsn)) { Write-Host 'Error: POSTGRES_DSN not found in .env file'; exit 1 }; $$file = '$(FILE)'; $$collections = \"$(COLLECTIONS)\"; if ($$collections -and $$collections.Trim()) { go run ./cmd/jsonimporter --file \"$$file\" --dsn \"$$dsn\" --lang es --public --collections \"$$collections\" } else { go run ./cmd/jsonimporter --file \"$$file\" --dsn \"$$dsn\" --lang es --public }"